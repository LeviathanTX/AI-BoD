        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {selectedAdvisors.length > 0 && (
            <div className="sticky top-0 z-10 mb-4">
              <div
                className={cn(
                  'px-4 py-2 rounded-lg flex items-center justify-between shadow-sm',
                  selectedMode === 'pitch_practice'
                    ? 'bg-gradient-to-r from-purple-100 via-pink-100 to-orange-100 border border-purple-200'
                    : selectedMode === 'strategic_planning'
                      ? 'bg-blue-50 border border-blue-200'
                      : selectedMode === 'due_diligence'
                        ? 'bg-green-50 border border-green-200'
                        : selectedMode === 'quick_consultation'
                          ? 'bg-orange-50 border border-orange-200'
                          : 'bg-purple-50 border border-purple-200'
                )}
              >
                <div className="flex items-center space-x-2">
                  {conversationModes.find(m => m.id === selectedMode)?.icon}
                  <span className="font-medium text-sm text-gray-900">
                    {conversationModes.find(m => m.id === selectedMode)?.name}
                  </span>
                  <span className="text-xs text-gray-500">
                    • {conversationModes.find(m => m.id === selectedMode)?.description}
                  </span>
                </div>
              </div>
            </div>
